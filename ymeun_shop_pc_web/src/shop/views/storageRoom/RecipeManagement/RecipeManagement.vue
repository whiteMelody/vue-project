...
<template>
    ...
    <div class="pal24 par24 pat20 pab20">
        <div class="bg-white pal20 par20">
            <p class="fc-gray4 f16px h40px lh40px">配方管理</p>
                                ...
                                <tr class="bg-white1">
                                    ...
                                    <td>
                                        <template v-if="item2.goodCount>0">
                                            <span class="fc-blue6 pointer" @click="$router.push({ path: 'RecipeDetail', query:{ row: JSON.stringify(item2), pname: item.items.name } })">修改</span>
                                        </template>
                                        <template v-else>
                                            <span class="fc-blue6 pointer" @click="$router.push({ path: 'RecipeDetail', query:{ row: JSON.stringify(item2), pname: item.items.name } })">新增</span>
                                        </template>
                                    </td>
                                </tr>
                            </template>
...
<script>
    ...
    import autoSearchCommodity from '@/shop/components/autocomplete/autoSearchCommodity'

    import { getList, getFormulaAdd } from '@/shop/assets/api/recipe/recipe'
