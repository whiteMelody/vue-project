  ...
  <div class="bg-white wb100 f14px">
      <div class="wb100">
        <p class="lh50px h50px">
          <span class="fl f16px fc-gray4">门店项目列表</span>
        </p>
...
<!--              <el-button type="primary" @click="dialogSetMoney = true">设置不计业绩</el-button>-->
            ...
            <base-list ref="demoTable" :condition="condition"
                         :datas.sync="list" :pageSearch="pageSearch" :isPaging="true"  v-on:listLoaded="getLoaded" :isAutoLoad="isAutoLoad">
    ...
    <!--                    <span class="mal10 mar10" style="color: #e5e5e5;">|</span>-->
          ...
          <!--   添加&修改下级类目   -->
          <edit-project ref="editProject" :isStore="false" :storeId="condition.storeId"></edit-project>
...
import { itemsList , itemsAdd , itemsChangeStatus, getListItems, saveValume, changeItemsValume } from "../../assets/api/projectManagement/projectManagement"
...
import EditProject from "./EditProject"
  ...
  export default {
    ...
    mounted() {
      ...
      // 项目
      getListItems({ itemsParentId: 0, storeId: this.condition.storeId, is_store_settings:1 }).then(res => {
        if(res.status == 0){
    ...
    methods: {
      ...
      searchData(){
        let list = this.list.filter(item => item.id == this.condition.itemsParentId)
        this.list = list
        if(list.length == 0 ){
          this.loadData()
        }
