  ...
  <div id="lotter_q3_order_page">
    ...
    <div class="wb100">
      ...
      <el-dialog
        ...
        <p class="Fgray-3 text-center F16 lh20px">{{cur.number}}期已到截止投注时间，您的投注将会投至{{next.number}}期</p>
        ...
        <template v-for="(item,key) in ticket">
          <template>
            <div class="pat15 pab15 par30 pal50 lh25px bor-gray bor-solid-1a F14 relative warpper-white">
              ...
              <router-link :to="{ name: 'LotteryQ3', query:{ ticketID: item.original.id }, params:{ playIndex: item.original.playType } }">
                <div class="wb100">
                  <p>
    ...
    methods: {
      ...
      myBack() {
        if (Base.isNull(this.call)) {
            this.$router.replace({ name: 'LotteryQ3', params:{ playIndex: this.playIndex } });
//            this.$router.go(-1);
        } else {
      ...
      conserve() {
        ...
        //保存成功
        this.$router.push({ name: 'LotteryQ3', params:{ playIndex: this.playIndex } });
      },
      clear() {
        document.getElementById("dialog").style.display = "none";
        //不保存
      ...
      clearAll() {
          ...
          window.sessionStorage.setItem('q3_ticket', JSON.stringify([]));
      ...
      randomSelect() {
        ...
        if (this.playIndex == 0) {
          ...
          _ticket.push(Base.group([1, 2, 3, 4, 5, 6], 3)[rd].join(''));
        } else if (this.playIndex == 3) {
          rd = Base.random_num(0, 29);
          var _arr = [{ num: 1, val: 11 }, { num: 2, val: 22 }, { num: 3, val: 33 }, { num: 4, val: 44 }, { num: 5, val: 55 }, { num: 6, val: 66 }];
          var _arr2 = [];
          for (var _i2 = 0; _i2 < _arr.length; _i2++) {
            for (var j = 1; j < 7; j++) {
              if (_arr[_i2].num != j) _arr2.push(_arr[_i2].val + '' + j);
            }
          }
        ...
        var _ticketArr = window.sessionStorage.getItem('q3_ticket');
        ...
        window.sessionStorage.setItem('q3_ticket', JSON.stringify(_ticketArr));
...
//                    console.log(ticket[i].numbers);
          ...
          if (_item.playType == 0) {
            if (_item.num == 1) {
              ...
              for (var _i3 = 0; _i3 < _item.numbers.length; _i3++) {
                _item.numbers[_i3] = Base.addZero(_item.numbers[_i3]);
              }
          ...
          if (_item.playType == 1) {
            //二不同号
            var _t = Base.group(_item.numbers, 2);
            var _num = [];
            for (var _i4 = 0; _i4 < _t.length; _i4++) {
              _num.push(_t[_i4].join(''));
            }
            _anteCodeList.push({
              playType: 313,
              anteCode: _num.join(',')
          ...
          if (_item.playType == 2 || _item.playType == 3 || _item.playType == 4) {
            ...
            if (_item.num == 1) {
              ...
              for (var _i5 = 0; _i5 < _item.numbers.length; _i5++) {
                _anteCodeList.push({
                  playType: 311,
                  anteCode: _item.numbers[_i5].split("").join(",")
                });
              }
            }
          }
          ...
          if (_item.playType == 5) {
            //二同号复选
            for (var _i6 = 0; _i6 < _item.numbers.length; _i6++) {
              _anteCodeList.push({
                playType: 314,
                anteCode: _item.numbers[_i6].substring(0, 1)
              });
            }
          }if (_item.playType == 6) {
            //三同号通选
        ...
        Interface.awardPeriod(this.lottery_type, (data) => {
          ...
          if (_number!=0) {
            ...
            Interface.k3Ticket(this.lottery_type, _number, this.multiple, _anteCodeList, (data, errorMsg) => {
