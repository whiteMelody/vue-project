<template>
  <div class="TaskList" :style="'min-height: '+boxMaxHeight+'px;'">
    <header>任务列表</header>
          ...
          <el-table-column align="center" label="操作">
            ...
            <template slot-scope="scope">
              <div class="buttomBox">
                ...
                <p class="buttonStyle_class" 
                   v-if="scope.row.type == 11 && scope.row.status.value == 0" 
                   @click="fillInClick(scope.row.type,scope.row.customer_id,scope.row.id,scope.row.appointment_id)">填写</p>
                <p class="buttonStyle" v-if="scope.row.type == 11 && scope.row.status.value == 1">已填写</p>

                <template v-if="allowed == 0">
                  <p class="buttonStyle_class"
                    v-show="scope.row.type == 3"
                    @click="fillInClick(scope.row.type,scope.row.customer_id,scope.row.id,scope.row.appointment_id,scope.row.time)"
                ...
                <template v-else> 
                  <p
                          class="buttonStyle_class"
                          v-show="scope.row.type == 2 || scope.row.type == 8"
                          @click="fillInClick(scope.row.type,scope.row.customer_id,scope.row.id,scope.row.appointment_id)"
                  >{{scope.row.status.value == 0 ? '填写' : '修改'}}</p>
                  <p
                          class="buttonStyle_class"
                ...
                <template v-if="scope.row.type == 21">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="$router.push({ path: 'OutStorageOrderCheck', query:{ id: scope.row.resources_id, taskId: scope.row.id }})">查看任务</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>

                <!-- 22调拨入库任务 -->
                <template v-if="scope.row.type == 22">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="$router.push({ path: 'InStorageOrderCheck', query:{ id: scope.row.resources_id, type: 2, taskId: scope.row.id }})">查看任务</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>

                <!-- 23调拨退货入库任务 -->
                <template v-if="scope.row.type == 23">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="$router.push({ path: 'InStorageOrderDetail', query:{ id: scope.row.resources_id, type: 3, taskId: scope.row.id }})">查看任务</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>

                <!-- 24采购审核任务 -->
                <template v-if="scope.row.type == 24">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="$router.push({ path: 'PurchaseOrderCheck', query:{ orderId: scope.row.resources_id, taskId: scope.row.id }})">查看任务</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>

                <!-- 25库存预警 -->
                <template v-if="scope.row.type == 25">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="finishTask(scope.row.id)">确认</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>

                <!-- 26到期预警 -->
                <template v-if="scope.row.type == 26">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="finishTask(scope.row.id)">确认</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>

                <!-- 27请货入库任务 -->
                <template v-if="scope.row.type == 27">
                  <p class="buttonStyle_class" v-if="scope.row.status.value == 0 || scope.row.status.value == 2" @click="$router.push({ path: 'InStorageOrderPleaseGoods', query:{ id: scope.row.resources_id, type: 4, taskId: scope.row.id }})">查看任务</p>
                  <p class="buttonStyle" v-else>已完成</p>
                </template>
...
export default {
  ...
  methods: {
    ...
    fillInClick(type, customerId, taskId,appointmentId,time) {
      if (type == 2) {
        this.$router.push({
          name: "ImprovingData",
        ...
        let str = time.substr(0,10);
        let dayStr = this.$base.getDateScope(1).startTime;
        dayStr = dayStr.substr(0,10);
        let obj = { customerId: customerId, taskId: taskId ,appointmentId:appointmentId}
        if (str != dayStr) {
          obj.mark = 1
        }
        this.$router.push({
