<template>
  ...
  <div class="TechnicianHome">
    <!-- <div class="card_data">
      ...
      <div class="card_list" v-loading="loading" v-if="!loading1">
    ...
    <div class="shop_data" v-loading="loading">
      ...
      <div class="shop_data_right">
        <div class="shop_data_right_box">
          <div class="text">
            总未划扣金额（元）
            <el-tooltip content="总未划扣金额绩统计的是门店所有剩余划扣总金额，不随查询条件变化而变化；" placement="bottom-start" effect="light"> 
    ...
    <div class="home_list_titles">
      ...
      <div class="home_list_box" v-loading="loading3">
    ...
    <div class="home_list_titles">
      <div class="titles">
        <p>
          <a>今日预约</a>
          <span>共计：{{appointmentTotal}}（条）</span>
        </p>
        <div @click="$router.push({name:'MakeAnAppointmentList'})" class="divXiaoshou">
          <strong>更多</strong>
          <i class="iconfont iconxiala"></i>
        </div>
      </div>
      <div class="home_list_box" v-loading="loading4">
        <el-table slot="list" :data="appointmentList" stripe border>
          ...
          <el-table-column prop="appointment_time" align="center" label="预约时间">
            <template slot-scope="scope">{{ scope.row.appointment_time }}</template>
          </el-table-column>
          <el-table-column prop="status_name" align="center" label="到店状态">
          ...
          <el-table-column prop="treatment_time" align="center" label="治疗时长/分">
            <template slot-scope="scope">{{ scope.row.treatment_time }}</template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    appointmentData() {
      this.loading4 = true;
      let appointmentData1 = {
        startTime: this.$base.getDateScope(1).startTime,
        endTime: this.$base.getDateScope(1).endTime,
        usersId: this.usersID
      };
      getAppointmentList(appointmentData1).then(res => {
        if (res.status == 0) {
    ...
    dataList() {
      ...
      getTargetList(timeData).then(res => {
        if (res.status == 0) {
          ...
          DeductionList.grayIsShow = false;
