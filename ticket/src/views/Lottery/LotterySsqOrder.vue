        ...
        <template v-for="(item,key) in ticket">
          ...
          <template v-if="item.red_Ball">
            ...
            <div class="pat15 pab15 par30 pal50 lh25px bor-gray bor-solid-1a F14 relative warpper-white">
              ...
              <p>
                <span class="Fred Fnum">（胆 {{item.red_Ball}}）</span>
              ...
              <router-link :to="{ name: 'LotterySsq', query:{ ticketID: item.original.id}, params:{ playIndex: 1 } }">
          ...
          <template v-else>
            ...
            <div class="pat15 pab15 pal50 par30 lh25px bor-gray bor-solid-1a F14 relative warpper-white">
              <div class="h50px w25px absolute" @click="removeTicket(item.original.id)" style="left: 15px; top: 15px;">
                <img src="~@/assets/images/icon-remove.png" style="float: left; margin-top: 15px; width: 20px;">
              </div>

              <router-link :to="{ name: 'LotterySsq', query:{ ticketID: item.original.id}, params:{ playIndex: 0 } }">
                <div class="">
                  <p>
                    <span class="Fred Fnum">{{item.redBall}}</span>
                    <span class="Fblue Fnum"> {{item.blueBall}}</span>
    ...
    methods: {
      init() {
        ...
        for (var i = 0; i < ticket.length; i++) {
          ...
          if (_index == -1) {
            //普通
            var _index2 = _item.indexOf('#');

            var _red = _item.substring(0, _index2);
            var _blue = _item.substring(_index2 + 1);
            ...
            //胆拖
            var _red1 = _item.substring(0, _index);
            var _red2_blue = _item.substring(_index + 1);

            var _index3 = _red2_blue.indexOf('#');

            var _red2 = _red2_blue.substring(0, _index3);
            var _blue2 = _red2_blue.substring(_index3 + 1);
      ...
      removeTicket(id) {
        Base.delTicketByID(id, 'ssq');
        this.init();
      },
      randomSelect() {

        var _rball = [];
        var _blue = Base.addZero(Base.random_num(1, 16));
        _rball.push(Base.addZero(Base.random_num(1, 33)));
        for (var i = 0; i < 5; i++) {
          //剩下5个红球
          var rd = Base.addZero(Base.random_num(1, 33));
          var _flag = true;
      ...
      emptyall() {
          ...
          window.sessionStorage.setItem('ssq_ticket', JSON.stringify([]))
...
//        }
        ...
        Interface.awardPeriod(this.lottery_type, (data) => {
          ...
          if (_tmp) {
            Interface.ssqTicket(_anteCodeList, _tmp.number, this.multiple, (data, errorMsg) => {
