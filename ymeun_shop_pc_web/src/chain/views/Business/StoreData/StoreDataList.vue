<template>
  ...
  <div class="wb100">
    <div class="pa18">
      <div class="bg-white pal18 par18 pab18">
        <div class="wb100 h60rem bor-gray bor-solid-1b">
            <div class="wb30 h60rem fl">
                <span class="Fgreen F18rem bor-solid-1b bor-green h60rem lh60rem fl">{{arrs[ arrs.length - 5 + curIndex ]}}实时数据</span>
            </div>
        ...
        >
            <el-table slot="list" :data="demoList"  border>
                ...
                <el-table-column prop="position" align="left" label="排名" width="50">
                    <template slot-scope="scope">{{ scope.row.position }}</template>
                </el-table-column>

                <el-table-column prop="name" align="left" label="门店名称">
                    <template slot-scope="scope">{{ scope.row.name }}</template>
                </el-table-column>

                <el-table-column prop="channel" align="left" label="销售业绩">
                    <template slot-scope="scope">{{ scope.row.order | _filterNumFormat }}</template>
                </el-table-column>

                <el-table-column prop="channel" align="left" label="划扣业绩">
                    <template slot-scope="scope">{{ scope.row.amount | _filterNumFormat }}</template>
                </el-table-column>

                <el-table-column align="left" label="操作" width="240">
                    <template slot-scope="scope">
                    <p class="operation">
                        <span class="fc-green2 pointer" @click="$router.push({ path: '/PerformanceShopDel',query: { id: scope.row.id }});">查看资料</span>
                        <span class="fc-gray mal10 mar10">|</span>
                        <span class="fc-blue pointer" v-if="scope.row.status == 0" @click="storeOpenOrCloseFn(2,scope.row.id)">启用</span>
                        <span class="fc-red5 pointer" v-else @click="storeOpenOrCloseFn(1,scope.row.id)">停用</span>
                    </p>
                    </template>
                </el-table-column>
            </el-table>
...
<style>
/*.ShopDeduct_box table{*/
/*    margin: 0;*/
/*    font-size: 16px;*/
/*}*/
/*.ShopDeduct_box .el-table--border tr th{*/
/*    padding: 8px 0;*/
/*}*/
/*.ShopDeduct_box .el-table__body-wrapper{*/
/*    margin-top:0;*/
/*}*/
/*.ShopDeduct_box .el-table__body-wrapper .el-table__empty-block{*/
/*    min-height: 40px*/
/*}*/
</style>
