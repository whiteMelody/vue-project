    ...
    <div class="wb100">
        ...
        <template v-for="(item,key) in ticket">
          <template>
            <div class="pat15 pab15 par30 pal50 lh25px bor-gray bor-solid-1a F14 relative warpper-white">
              ...
              <router-link :to="{ name: 'LotteryDlt', query:{ ticketID: item.original.id }, params:{ playIndex: 0 } }">

                <div class="wb100">
                  <p>

                    <template v-if="item.redBall2!='' && item.redBall2!=' '">
                      <!--胆拖玩法-->
                      <span class="Fred Fnum">（胆 {{item.redBall}}） {{item.redBall2}}</span>
                    </template>

                    <template v-else>
                      <!--普通玩法-->
                      <span class="Fred Fnum">{{item.redBall}}</span>
                    </template>

                    <template v-if="item.blueBall2!='' && item.blueBall2!=' '">
                      <!--胆拖玩法-->
                      <span class="Fblue Fnum">（胆 {{item.blueBall}}） {{item.blueBall2}}</span>
                    </template>

                    <template v-else>
                      <!--普通玩法-->
                      <span class="Fblue Fnum">{{item.blueBall}}</span>
                    </template>
      ...
      <div class="wb100 bor-gray bor-solid-1t fixed warpper-white" style="bottom: 0;">
        ...
        <div class="pal15 par15 h30px lh30px pat5 pab5 warpper-gray-1 bor-gray bor-solid-1t bor-solid-1b relative">
          ...
          <div class="w85px fr F14" style="width: 83px;" @click="subjoin = !subjoin; init()">

            <div class="fl w20px h20px mat5">
              <div class="wb100 w18px h18px pa1" v-if="subjoin == true" :style="'background: url('+ checkBg +'); background-size:100% 100%; '"></div>
              <div class="w18px bor-black1 bor-solid-1a radius5 warpper-white h18px" v-else></div>
              <!--i-check-->
            </div>

            <div class="w60px h30px lh30px fr">
              追加投注
            </div>

            <!--<el-checkbox v-model="subjoin"></el-checkbox>-->
...
<script>
  ...
  import Bg2 from '../../assets/images/i-check.png'
    ...
    methods: {
      ...
      init() {
        ...
        for (var i = 0; i < ticket.length; i++) {
          ...
          var _item = ticket[i].numbers;
          var _num = ticket[i].num;

          var _index = _item.indexOf('#');
          var _red = _item.substring(0, _index);
          var _blue = _item.substring(_index + 1);
          var _redIndex = _red.indexOf('$');
          var _blueIndex = _blue.indexOf('$');
          ...
          if (_redIndex == -1 && _blueIndex == -1) {
            ...
            if(this.subjoin == true){
              _title = '追加玩法 ' + _num + '注 ' + _num * 3 + '元';
            }else{
      ...
      conserve() {
        ...
        //保存成功
        this.$router.push({ name: 'LotteryDlt' });
      },
      clear() {
        document.getElementById("dialog").style.display = "none";
        //不保存
      ...
      calculate() {
        ...
        if(this.subjoin == true){
          this.sumPrice = _sumNum * 3 * _sumMultiple * this.periods;
        }else{
      ...
      randomSelect() {
        ...
        for (var i = 0; i < 4; i++) {
          //剩下5个红球
          var rd = Base.addZero(Base.random_num(0, 34));
          var _flag = true;
        ...
        for (var _i2 = 0; _i2 < 1; _i2++) {
          var _rd = Base.addZero(Base.random_num(0, 11));
          var _flag2 = true;
          for (var _i3 = 0; _i3 < _bball.length; _i3++) {
            //去重
            if (_rd == _bball[_i3]) {
              _flag2 = false;
            }
        ...
        var _result = _rball.join(',') + '#' + _bball;
      ...
      //清空全部
      emptyall() {
        window.sessionStorage.setItem('dlt_ticket', JSON.stringify([]));
        this.init();
        this.calculate();
      },
      submitOrder() {
...
//        }
        ...
        for (var i = 0; i < ticket.length; i++) {
          ...
          if(this.subjoin == true){
            if(ticket[i].type == 101){
              ticket[i].type = 103;
            }
            if(ticket[i].type == 102){
              ticket[i].type = 104;
            }
            if(ticket[i].type == 106){
              ticket[i].type = 107;
            }
          }else{
            if(ticket[i].type == 103){
              ticket[i].type = 101;
            }
            if(ticket[i].type == 104){
              ticket[i].type = 102;
            }
            if(ticket[i].type == 107){
              ticket[i].type = 106;
            }
          }
        ...
        Interface.awardPeriod(this.lottery_type, (data) => {
          ...
          if (_tmp) {
            Interface.dltTicket(_tmp.number,  this.multiple, _subjoin, _anteCodeList, (data, errorMsg) => {
