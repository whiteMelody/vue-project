<template>
  <el-dialog :title="form.p_name" :visible.sync="dialogForm" width="380px" class="myDialog">
    ...
    <div class="pal20 par20">
      <el-form ref="myForm" :model="form" :rules="rules">
          <!--<el-form-item label="一级类名" prop="name">-->
              <!--<el-select v-model="form.p_id" placeholder="一级类名" readonly clearable disabled class="w184px">-->
            ...
            <el-form-item label="下级名称" prop="name">
              <el-input type="text" v-model.trim="form.name" class="w184px" placeholder="请输入名称" clearable></el-input>
            </el-form-item>
      ...
      dialogForm(bool) {
        if (!bool) {
          ...
          this.isEdit = false
            ...
            if(!this.isStore){
                getListChannel({ channelParentId: 0, storeId: this.storeId, is_store_settings:1,chain:1 }).then(res => {
                    if(res.status == 0){
                ...
                getListChannel({ channelParentId: 0,chain:1 }).then(res => {
                    if(res.status == 0){
