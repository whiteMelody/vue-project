  ...
  <section class="base-list custom-input">
    ...
    <el-row class="bottom" type="flex" v-show="isPaging && total > pageInfo.size">
      ...
      <el-pagination class="page" @size-change="size=>pageInfo.size=size"
                      :disabled="isPageDisabled" @current-change="pageInfoFn"
                     layout="total, sizes, prev, pager, next, jumper"
                     :current-page.sync="pageInfo.pageStart" :page-sizes="isPageSize ? pageInfo.pageArr : [15] "
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pageInfoFn(start){
        this.$emit('pageChange', this.pageInfo.pageStart)
      },
      
      startLoadData(isPageStr,start){
        if(isPageStr){
          this.loadData()
          this.sPageStart = start
        }
        else{
          this.pageInfo.pageStart = this.sPageStart
        }
      }
