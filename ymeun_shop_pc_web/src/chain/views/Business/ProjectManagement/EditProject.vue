<template>
  ...
  <el-dialog :title="form.p_name" :visible.sync="dialogForm" width="1000px">
    <div class="dialog_xian"></div>
    <div>
      <el-form ref="myForm" :model="form" :rules="rules" label-width="100px" label-position="right">
          ...
          <template v-else>
            ...
            <div class="w320px fl">
                <el-form-item label="品项" prop="fashion" label-width="50px">
            ...
            <div class="w320px fl">
                <el-form-item label="下级名称" prop="name">
                    <el-input type="text" v-model.trim="form.name" placeholder="请输入下级名称" clearable></el-input>
                </el-form-item>
            </div>
            <div class="w320px fl">
              <el-form-item label="下级简称" prop="alias_name">
                  <el-input type="text" v-model.trim="form.alias_name" placeholder="请输入下级简称" clearable></el-input>
              </el-form-item>
            </div>
            <div class="w320px fl">
                <el-form-item label="价格" prop="price" label-width="50px">
                    <template v-if="form.style == 1">
                        <el-input type="text" v-model.trim="form.price" placeholder="" clearable disabled></el-input>
                        <p class="fc-red f12px">总价格为套餐价格之和</p>
                    </template>
                    <template v-else>
                        <el-input type="text" v-model.trim="form.price" placeholder="请输入价格" clearable></el-input>
                    </template>
                </el-form-item>
            </div>
            <div class="w320px fl">
                <el-form-item label="疗程次数" prop="treatment_num">
                    <template v-if="form.style == 1">
                        <el-input type="text" v-model.trim="form.treatment_num" placeholder="" clearable  disabled></el-input>
                        <p class="fc-red f12px">总疗程次数为套餐疗程次数之和</p>
                    </template>
                    <template v-else>
                        <el-input type="text" v-model.trim="form.treatment_num" placeholder="请输入疗程次数" clearable ></el-input>
                    </template>
                </el-form-item>
            </div>
          ...
          <div class="clear0"></div>
            ...
            <template v-if="form.style == 1">
                ...
                <template v-for="(item, index) in comboList">
                    ...
                    <div class="pat10">
                        <div class="w320px fl">
                            <el-form-item :label="'套餐'+(index + 1)+'信息'" label-width="77px">
                        ...
                        <div class="w320px fl">
                            <el-form-item label="疗程次数" prop="treatment_num" label-width="79px">
...
import { itemsList , itemsAdd , itemsChangeStatus, getListItems } from "@/chain/assets/api/projectManagement/projectManagement"
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
			p_id: [{required: true, message: '请选择一级类目', trigger: 'blur'}],
			name: [{required: true, message: '请输入品项名称', trigger: 'blur'}],
            alias_name: [{required: true, message: '请输入品项简称', trigger: 'blur'}],
      ...
      comboList: {
          ...
          handler: function() {
              ...
              if(this.comboList.length > 0){
                  ...
                  this.form.price = isNaN(_sumPrice) ? 0 : _sumPrice
                  this.form.treatment_num = isNaN(_sumNum) ? 0 : _sumNum
              }
...
.el-dialog .el-dialog__body .dialog_xian{
  padding-top: 14px;
  border-top: 1px solid #e5e5e5;
}
.timer_border .el-date-editor input{
  border: 0 !important;
