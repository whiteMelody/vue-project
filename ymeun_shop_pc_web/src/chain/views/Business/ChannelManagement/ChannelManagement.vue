<template>
  <div class="bg-white wb100 f14px ChannelManagement">
      <div class="wb100">
        <p class="lh40px h40px pal20 par20">
          <span class="fl f14px fc-gray4">连锁渠道列表</span>
        </p>
        <div class="clear0 bd-b1 bd-gray6"></div>
        ...
        <div class="pal20 par20">
                ...
                <el-table-column align="left" label="操作">
                  <template slot-scope="scope">
                    <p v-if="scope.row.items" class="pointer">
                      ...
                      <span class="fc-green2 fl" @click="showDialog(false, scope.row)">修改</span>
                      <span class="mal10 mar10 fc-gray fl">|</span>
          ...
          <!--   添加&修改大类弹窗   -->
          <el-dialog :title="isEdit ? '修改大类' : '添加大类'" :visible.sync="dialogEditCategories" width="335px">
            <div class="dialog_xian"></div>
            <el-form :model="dialogEditCategoriesParam" label-width="70px">
...
import EditChannel from "./EditChannel"
  ...
  export default {
    ...
    methods: {
      ...
      getLoaded(data){
        let _tmp = data.list.filter(item => item.chain == 1);

        _tmp = data.list.filter(item => item.status.value != 0)
      ...
      loadData() {
        ...
        getListChannel({ channelParentId: 0 , status: -1}).then(res => {
          if(res.status == 0){
      ...
      handleDialogFormOpen(isEdit, item) {
        ...
        // this.$refs.editChannel.dialogForm = true;
         ...
         if (isEdit) {
          ...
          if(row.meal){
            this.$refs.editChannel.comboList = JSON.parse(JSON.stringify(row.meal));
          }else{
            this.$refs.editChannel.comboList = []
          }
          ...
          this.$refs.editChannel.selectAll = false;
          this.$refs.editChannel.comboList = []
        }
        this.$refs.editChannel.dialogForm = true;
      },
...
<style>
.ChannelManagement .el-dialog .el-dialog__header{
  padding: 10px 20px;
}
.ChannelManagement .el-dialog .el-dialog__header .el-dialog__title{
  font-size: 16px;
}
.ChannelManagement .el-dialog .el-dialog__body{
  padding: 0 20px
}

.ChannelManagement .dialog_box{
  display: flex;
  flex-wrap: wrap;
  padding-top: 
}
.ChannelManagement .dialog_xian {
  height: 1px;
  background: #E5E5E5;
  margin-bottom: 14px;
}
