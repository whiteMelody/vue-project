...
<template>
    ...
    <div class="pal24 par24 pat20 pab20">
        <div class="bg-white pal20 par20">
            <p class="fc-gray4 f14px h40px lh40px">采购订单列表</p>
            ...
            <base-list :condition="condition" ref="myList"
                ...
                <el-table slot="list" :data="demoList" border>
                    ...
                    <el-table-column align="left" label="采购数量合计">
                        <template slot-scope="scope">
                            {{scope.row.quantity | _filterNumFormat }}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="采购状态" width="100px">
                        <template slot-scope="scope">
                    ...
                    <el-table-column align="left" label="操作" width="120px">
                        <template slot-scope="scope">
                            <span class="fc-blue pointer" @click="$router.push({ path: 'PurchaseOrder', query:{ orderId: scope.row.id } })">查看详情</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column align="left" label="审核订单" width="120px">
                        <template slot-scope="scope">
                            <template v-if="scope.row.status.value == 1">
                                <span class="fc-green2 pointer" @click="$router.push({ path: 'PurchaseOrderCheck', query:{ orderId: scope.row.id } })">审核订单</span>
                            </template>
                            <template v-else>
...
<script>
    ...
    import { getList } from '@/chain/assets/api/purchase/purchase'
