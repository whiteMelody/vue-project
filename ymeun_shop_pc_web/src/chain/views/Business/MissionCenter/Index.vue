  ...
  <div class="MissionCenter" v-loading="loading" :style="'min-height: '+boxMaxHeight+'px;'">
    ...
    <div class="listBox">
      <el-table slot="list" :data="demoList" border>
        ...
        <el-table-column align="left" label="操作" v-if="allowed == 1">
          <template slot-scope="scope">
...
import { getMissionCenter } from "@/chain/assets/api/task/task";
import Card from "@/chain/components/home/Card";
export default {
  name: "MissionCenter",
  data() {
    return {
  ...
  methods: {
    ...
    IndexData() {
      this.loading = true;
      getMissionCenter({ mark: this.mark,storeId:this.id }).then(res => {
        if (res.status == 0) {
          // 列表数据
  ...
  mounted() {
    ...
    getProgramList().then(res=>{
      if(res.status == 0){
        if(res.data.list.rwzx.allowed == 0){
          this.allowed = 0
        }else{
          this.allowed = 1
        }
