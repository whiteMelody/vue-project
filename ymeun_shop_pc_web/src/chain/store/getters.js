...
const getters = {
    ...
    projectCascaderData: (state) => {
        let _list = getters.projectListNoEmpty(state)
        let _arr = []
        for(let i=0; i<_list.length; i++){
            ...
            if(_list[i].children){
                for(let j=0; j<_list[i].children.length; j++){
                    _tmp.children.push({
                        value: _list[i].children[j].id,
                        label: _list[i].children[j].name,
                    })
                }
            }
            _arr.push(_tmp)
    ...
    projectList11Map: (state) => (type, items_volume) => {
        ...
        if(projectList.length){
            ...
            if(type == 1){
                ...
                for(let i=0; i<_tmp.length; i++){
                    if(_tmp[i].children){
                        let _t = _tmp[i].children.filter(item => item.disabled_volume == 0 && items_volume < item.price)
                        if(_t.length == 0){
                            _tmp.splice(i, 1)
                        }
                    }
    ...
    projectList22: (state) => (id, type, items_volume) => {
        ...
        if(projectList.length){
            ...
            if(_t){
                if(type){
                    if(type == 1){
                        return _t.children.filter(item => item.disabled_volume == 1 || items_volume >= item.price)
                    }else{
                        return _t.children.filter(item => item.disabled_volume == 0 &&  items_volume < item.price )
                    }
                }
                return _t.children.filter(item => item.disabled_volume == 0 &&  items_volume < item.price )
            }
    ...
    channelCascaderData: (state) => {
        let _list = getters.channelListNoEmpty(state)
        let _arr = []
        for(let i=0; i<_list.length; i++){
            let _tmp = {
                value: _list[i].id,
