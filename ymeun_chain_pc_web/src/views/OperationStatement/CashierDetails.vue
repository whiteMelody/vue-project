    ...
    <div class="box">
           ...
           <el-form :inline="true" :model="condition" class="search-form" slot="search" label-width="80px">
              ...
              <el-form-item  label="">
               ...
               <!--  选择员工 -->
              <el-form-item label="">
                  <el-select v-model="condition.adviserId" placeholder="选择顾问" readonly="" clearable :disabled="!condition.storeId">
          ...
          <div class="pa20 bg-pink2 text-center f16px">
            ...
            <div class="wb11 fl h40px lh40px bd-red3 bd-r1 mat15 f16px">
              收银数据合计
            </div>

            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">应收金额（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.payment | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">余额支付（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.rechargeDebit | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">券额支付（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.giveDebit | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">欠款（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.arrearsPrice | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">还款总额（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.huankuan_price | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">退款总额（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.refund_price | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">充值总额（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.prepaid_price | _filterNumFormat}}</p>
            </div>
            <div class="wb11 fl">
              <p class="fc-gray2 h20px lh20px">实收金额（元）</p>
              <div class="clear2"></div>
              <p class="fc-gray3 f20px h40px lh40px">{{userData.ActualPrice | _filterNumFormat}}</p>
            </div>
        ...
        <div class="pal20 lh38px h40px pointer">
            <template v-if="showType == 0">
                <span class="fl fc-green2 f16px bd-green2 bd-b2">收银明细</span>
            ...
            <template v-else>
                ...
                <span class="fl f16px mal20 mar20 fc-gray1"> | </span>
                <span class="fl fc-green2 f16px bd-green2 bd-b2">不计入业绩明细</span>
            </template>
        </div>
        ...
        <base-list ref="demoTable" :condition="condition"
          ...
          <el-table slot="list" :data="demoList" stripe border
           ...
           >
            ...
            <el-table-column prop="items_balance" align="center" label="项目余额支付">
              <template slot-scope="scope">
                {{ scope.row.items_balance ? _filterNumFormat(scope.row.items_balance) : '' }}
              </template>
            </el-table-column>

            <el-table-column prop="items_give_balance" align="center" label="项目券额支付">
              <template slot-scope="scope">
        ...
        <el-button type="primary" @click="deriveExcel(2)">项目明细导出</el-button>
    </el-dialog>
...
<script>
import { getCashierDetails } from "@/assets/api/manage/statistics"
