<template>
    <div class="mal20 mar20 bg-white mat24" :style="'min-height: '+boxMaxHeight+'px;'">
        <p class="lh50px h50px pal24 par24 bd-b1 bd-gray">
            <span class="fl f16px fc-gray4">合作企业开单列表</span>
        </p>

        <div class="clear4"></div>

        <div class="wb100 pal24 par24">
            ...
            <div class="wb100">
                ...
                <el-table slot="list" :data="demoList" border>
                    ...
                    <el-table-column align="left" label="合作企业名称">
                        <template slot-scope="scope">
                            {{scope.row.businessName.name}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="开单时间">
                        <template slot-scope="scope">
                            {{scope.row.time}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="开单人及百分比">
                        <template slot-scope="scope">
                    ...
                    <el-table-column align="left" label="合作业绩">
                        <template slot-scope="scope">
                            {{scope.row.price | _moneyFormat}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="门店占比(%)">
                        <template slot-scope="scope">
                            {{scope.row.enterpriseRatio}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="门店提成金额">
                        <template slot-scope="scope">
                            {{scope.row.enterprisePrice | _moneyFormat}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="合作企业占比(%)">
                        <template slot-scope="scope">
                            {{scope.row.ratio}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="合作企业提成金额">
                        <template slot-scope="scope">
                            {{scope.row.businessPrice | _moneyFormat}}
                        </template>
                    </el-table-column>

                    <el-table-column align="left" label="门店提成金额占比(%)">
                        <template slot-scope="scope">
...
<script>
    ...
    import {  getStoreMembers, ordersList, businessList } from "@/chain/assets/api/common/common"
        ...
        methods: {
            ...
            deriveExcel() {
                this.$postRequest("/post/api/getUserInfo").then(res => {
                    ...
                    url += `&startTime=${params.startTime}&stopTime=${params.endTime}`;
                    if (params.storeId) {
...
.CooperativeEnterpriseList .rankingTitle a{
