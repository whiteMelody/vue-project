...
<template>
    ...
    <div class="pal24 par24 pat20 pab20">
        <div class="bg-white pal20 par20">
            <p class="fc-gray4 f16px h40px lh40px">{{isEdit ? '商品详情' : '新增商品'}}</p>
            ...
            <div class="wb100" v-if="isEdit">
                ...
                <template v-if="$store.getters.currentUser.compose.id == 2">
                ...
                <el-form :inline="true" ref="form" :rules="rules" :model="form" label-width="200px">
                    ...
                    <div class="wb50 fl">
                        <el-form-item label="商品名称" prop="fullname">
                            <el-input v-model="form.fullname" class="w188px" :disabled="isEdit && $store.getters.currentUser.compose.id != 2" placeholder="请输入商品名称"></el-input>
                        </el-form-item>
                    </div>

                    <div class="wb50 fl">
                        <el-form-item label="商品简称" prop="title">
                            <el-input v-model="form.title" class="w188px" :disabled="isEdit && $store.getters.currentUser.compose.id != 2" placeholder="请输入商品简称"></el-input>
                        </el-form-item>
                    </div>

                    <div class="wb50 fl">
                        <el-form-item label="商品类别" prop="catalogue_id">
                            <el-select placeholder="请选择商品类别" v-model="form.catalogue_id" class="w188px" :disabled="isEdit && $store.getters.currentUser.compose.id != 2">
                                <el-option v-for="item in catalogueList" :key="item.title" :label="item.title"
                                           :value="item.title"></el-option>
                            </el-select>
<!--                            <auto-search-catalogue @changed="catalogueChanged" class="w188px" :disabled="isEdit && $store.getters.currentUser.compose.id != 2" v-if="loaded" :initValue="form.catalogue.title"></auto-search-catalogue>-->
                        </el-form-item>
                    </div>

                    <div class="wb50 fl">
                        <el-form-item label="供应商" prop="business_id">
                            <el-select placeholder="请选择供应商" v-model="form.business_id" class="w188px" :disabled="isEdit && $store.getters.currentUser.compose.id != 2">
                    ...
                    <div class="wb50 fl">
                        <el-form-item label="商品单位" prop="unit_id">
                            <el-input :value="form.unit.title" class="w188px" disabled v-if="isEdit && $store.getters.currentUser.compose.id != 2"></el-input>
                            <auto-search-unit @changed="unitChanged" class="w188px" v-else :initValue="form.unit.title"></auto-search-unit>
                        </el-form-item>
                    </div>

                    <div class="wb50 fl">
                        <el-form-item label="商品规格" prop="specifications_id">
                            <el-input :value="form.specifications.title" class="w188px" disabled v-if="isEdit && $store.getters.currentUser.compose.id != 2"></el-input>
                            <auto-search-specification @changed="specificationChanged" class="w188px" v-else :initValue="form.specifications.title"></auto-search-specification>
                        </el-form-item>
                    </div>

                    <div class="wb50 fl">
                        <el-form-item label="税率" prop="tax">
                            <el-input-number class="w188px fl mar10" :disabled="isEdit && $store.getters.currentUser.compose.id != 2" v-model="form.tax" :min="0" :controls="false" :precision="2" placeholder="请输入税率"></el-input-number>
                            <span class="fl">%</span>
                        </el-form-item>
                    </div>
                    ...
                    <div class="wb50 fl relative">
                        <el-form-item label="库存预警" prop="stocks_warning">
                            <el-input v-model="form.stocks_warning" class="w188px" placeholder="请输入库存预警数量" :disabled="isEdit && $store.getters.currentUser.compose.id != 2"></el-input>
                    ...
                    <div class="wb50 fl">
                        <el-form-item label="到期预警" prop="expire_warning">
                            <el-input v-model="form.expire_warning" placeholder="请输入到期预警天数" class="w188px fl mar10" :disabled="isEdit && $store.getters.currentUser.compose.id != 2"></el-input> <span class="fl">天</span>
                            <div class="clear0"></div>
                    ...
                    <div class="wb100">
                        <el-form-item label="备注">
                            <el-input type="textarea" v-model="form.remarks" :style="'width:'+myWidth+'px'" :disabled="isEdit && $store.getters.currentUser.compose.id != 2"> </el-input>
                        </el-form-item>
                    </div>
                ...
                <div class="wb100 text-right">
                    <el-button class="primary" @click="back">取消</el-button>
                    ...
                    <template v-else>
                        <template v-if="$store.getters.currentUser.compose.id == 2">
                            <el-button type="primary" @click="submit"> {{isEdit ? '保存修改' : '确认新增'}} </el-button>
                        </template>
                        <template v-else>
                            <el-button type="primary" @click="submit" v-if="!isEdit">确认新增</el-button>
                        </template>
                    </template>
                </div>
...
<script>
    ...
    import { save, getSpecifications, getUnit, getCatalogue, getInfo } from '@/shop/assets/api/store/store'

    import autoSearchSpecification from '@/shop/components/autocomplete/autoSearchSpecification'
    import autoSearchCatalogue from '@/shop/components/autocomplete/autoSearchCatalogue'
    import autoSearchUnit from '@/shop/components/autocomplete/autoSearchUnit'
    ...
    export default {
        ...
        data() {
            return {
                ...
                disabled: false,
                action: this.$base.getStoreHost('store') + '/post/attachment/document/upload',
                good_id: '',
                loaded: false,
                isEdit: false,
                catalogueList: [],
        ...
        mounted() {
            ...
            this.myWidth = (window.innerWidth + 188) / 2;
