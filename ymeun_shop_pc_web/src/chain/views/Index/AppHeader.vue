<template>
  <div class="wb100 h35px lh35px f16px bd-gray bd-b1 appHeader">
    ...
    <div class="pal20 par20 fc-gray4">
      ...
      <div class="w50px h35px lh35px fr text-center mal20 mar20 pointer myNav" title="通知">
        <i class="iconfont iconxiaoxi f18px" @click="$router.push({name:'MessageList'})">
          <el-badge v-show="$store.getters.messageCount > 0" class="mark" :value="$store.getters.messageCount"/>
        </i>
      </div>
      <div class="w50px h35px lh35px fr text-center pointer myNav" title="任务">
        <span class="el-dropdown-link">
          <i class="iconfont iconrenwu f18px" @click="$router.push({name:'TaskList'})">
...
<script>
import { getUnreadMessage } from "@/chain/assets/api/message/message";
import { editPassword, logout, getUserInfo } from "@/chain/assets/api/common/common";
import { taskListChain  } from "@/chain/assets/api/task/task"
