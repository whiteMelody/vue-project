<template>
  <div class="CosmetologistTarget wb100">
    <p class="lh50px h50px">
      <span class="fl f16px fc-gray4">{{$store.getters.roleName.technician}}目标况表</span>
    ...
    <div class="wb100">
      ...
      <div class="wb100">
        ...
        <div class="pa20 bg-blue5 text-center bd-blue4 bd-a1 f16px">
          ...
          <div class="wb15 fl h40px lh40px bd-gray bd-r1 mat15">
            {{$store.getters.roleName.technician}}目标合计
          </div>
          ...
          <div class="wb14 fl">
            ...
            <div class="clear2"></div>
            <p class="fc-blue6 f20px h40px lh40px">{{info.consumePrice | _filterNumFormat}}</p>
          </div>
          <div class="wb14 fl">
        ...
        <base-list ref="demoTable" :condition="condition"
          ...
          <el-table slot="list" :data="demoList" stripe border>
            ...
            <el-table-column prop="name" align="center" :label="$store.getters.roleName.technician">
            ...
            <el-table-column prop="actual.consumePrice" align="center" label="划扣业绩">
              <template slot-scope="scope">
                {{ scope.row.actual.consumePrice | _filterNumFormat }}
              </template>
            </el-table-column>
...
<script>
  ...
  import { getTechnicianPayment } from "@/shop/assets/api/manage/statistics"

  export default {
    name: "CosmetologistTarget",
    components: {
      dateSlot
    },
    data() {
      return {
        ...
        demoList: [],                      // table数组
        info:{},
        pageSearch: getTechnicianPayment,       // 分页查询api接口
      }
    },
    watch: {
      dateTime(c, o){
    ...
    methods: {
      ...
      deriveExcel() {
        ...
        if(window.location.hostname == this.$base.getStoreHost()){
          url = `https://${location.hostname}/post/export/getTechnicianPaymentExcel?downloadUserId=${this.$store.getters.currentUser.users_id}`;
        }else{
          url = `http://${location.hostname}:${location.port}/post/export/getTechnicianPaymentExcel?downloadUserId=${this.$store.getters.currentUser.users_id}`;
        }
