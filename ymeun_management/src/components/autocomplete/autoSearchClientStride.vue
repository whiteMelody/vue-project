...
<script>
  ...
  import {customerList, getListByKeyword} from  "@/assets/api/client/client"
  ...
  export default {
    ...
    methods: {
      ...
      createStateFilter(queryString) {
        return (state) => {
          if(this.$base.isNull(state.value))
      ...
      loadData(){
        ...
        getListByKeyword({keyword: this.searchValue, perpage: 9999, crossStore: 1}).then((res)=>{
          if(res.status == 0){
