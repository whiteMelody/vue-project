...
<template>
    ...
    <div class="pal24 par24 pat20 pab20">
        <div class="bg-white pal20 par20">
            <p class="fc-gray4 f16px h40px lh40px">出库列表</p>
            ...
            <el-form :inline="true" :model="condition">
                ...
                <el-form-item>
                    <el-button type="primary" class="h32px" @click="$router.push({ path: 'OutStorageOrderList'})">新增出库单据</el-button>
                </el-form-item>
            ...
            <base-list :condition="condition" ref="myList"
                ...
                <el-table slot="list" :data="demoList" border>
                    ...
                    <el-table-column align="center" label="出库类别">
                        <template slot-scope="scope">
                            {{scope.row.type.name}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column align="center" label="操作" width="120px">
                        <template slot-scope="scope">
                            <span class="fc-blue6 pointer" @click="$router.push({ path: 'outStorageOrderDetail', query:{ id: scope.row.id } })">查看详情</span>
                        </template>
                    </el-table-column>
                        ...
                        <template slot-scope="scope">
                            <template v-if="scope.row.status.id == 1">
                                <template v-if="$store.getters.currentUser.role_id == 2">
                                    <span class="fc-blue6 pointer" @click="$router.push({ path: 'OutStorageOrderCheck', query:{ id: scope.row.id } })">审核订单</span>
                                </template>
                                <template v-else>
...
<script>
    ...
    import { outboundList } from '@/shop/assets/api/outStorage/outStorage'
