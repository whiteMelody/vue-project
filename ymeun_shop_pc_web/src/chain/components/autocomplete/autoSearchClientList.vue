...
<script>
  ...
  import {appointmentList} from  "@/chain/assets/api/client/client"
  ...
  export default {
    ...
    methods: {
      ...
      loadData(){
        ...
        appointmentList({data: this.searchValue, perpage: 9999}).then((res)=>{
          if(res.status == 0){
            this.list = res.data.list
            this.restaurants = formatArrayToMap(res.data.list, 'id', 'customer_name')
            this.$emit("changed", {
              kw: this.searchValue,
              list: res.data.list
            })
